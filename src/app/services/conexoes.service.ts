...
// Conexões por credencial (Shopee/Amazon). O Mercado Livre não entra aqui:
// ele conecta por sessão de navegador, não por credencial (ver ConnectionCard).
...
export class ConexoesService {
  ...
  async carregar(): Promise<void> {
    try {
      ...
      // silencioso — a tela mostra o estado que já tinha; o usuário pode
      // tentar conectar de novo, que aí o erro aparece no próprio card.
    }
  }
  ...
  async desconectar(provedor: Provedor): Promise<void> {
    try {
      ...
      // se falhar, o estado na tela continua o mesmo — nada some sem confirmação
    }
  }
}
